...
from generator.db import init_db, Activity, Base
...
def display_all_activities(session):
    ...
    print(f"{'='*120}\n")
    print(f"{'ID':<8} {'Name':<30} {'Date':<20} {'Type':<12} {'Distance':<10} {'Source':<10}")
    print("-" * 120)
    ...
    for activity in activities:
        ...
        print(f"{activity_id:<8} {name:<30} {date:<20} {activity_type:<12} {distance:<10} {source:<10}")
...
def display_activity_by_id(session, run_id):
    ...
    print(f"Distance:            {activity.distance} km" if activity.distance else "N/A")
    print(f"Moving Time:         {activity.moving_time}")
    print(f"Elapsed Time:        {activity.elapsed_time}")
    print(f"Average Speed:       {activity.average_speed} m/s" if activity.average_speed else "N/A")
    print(f"Average Heart Rate:  {activity.average_heartrate}" if activity.average_heartrate else "N/A")
    print(f"Elevation Gain:      {activity.elevation_gain} m" if activity.elevation_gain else "N/A")
...
def display_by_date_range(session, start_date, end_date):
    ...
    print(f"\n{'='*120}")
    print(f"Activities from {start_date} to {end_date} ({len(activities)} found)")
    print(f"{'='*120}\n")
    print(f"{'ID':<8} {'Name':<30} {'Date':<20} {'Type':<12} {'Distance':<10} {'Source':<10}")
    print("-" * 120)
...
def delete_activity_by_id(session, run_id):
    ...
    confirm = input("\nAre you sure you want to delete this activity? (yes/no): ").lower().strip()
...
def delete_activities_by_ids(session, ids):
    ...
    confirm = input("\nAre you sure you want to delete these activities? (yes/no): ").lower().strip()
...
def delete_by_date_range(session, start_date, end_date):
    ...
    confirm = input("\nAre you sure you want to delete all these activities? (yes/no): ").lower().strip()
    ...
    try:
        ...
        while True:
            ...
            if choice == "1":
                ...
                try:
                    ids_input = input("Enter activity ID(s) to delete (comma or space separated): ").strip()
                    if not ids_input:
                    ...
                    print("Invalid ID(s). Please enter number(s) separated by commas or spaces.")
